...
class Client(NumPyClient):
        ...
        if fit_config["validation_split"] > 0:
            for metric_name in training_metrics_history.history.keys():
                if "val_" in metric_name:
                    training_metrics[metric_name] = training_metrics_history.history[metric_name][
                        -1
                    ]
    ...
    def evaluate(
        ...
        message = (
            "[Client {0} | FL Round {1}] Finished Evaluating the Model in {2} Seconds.".format(
...
class FlowerClient:
    ...
    def get_grpc_server_ip_address_and_port(self) -> str:
        ...
        return (
            grpc_settings["grpc_server_ip_address"] + ":" + str(grpc_settings["grpc_server_port"])
        )
    ...
    @staticmethod
    def derive_images_attributes(x_phase_folder: Path, y_phase_labels_file: Path) -> tuple:
    ...
    def load_ml_model_private_dataset(self) -> None:
        ...
        message = "[Client {0}] Loading the '{1}' Private Dataset ({2} Storage)...".format(
...
def main() -> None:
    ...
    ag.add_argument("--client_id", type=int, required=True, help="Client ID (no default)")
    ag.add_argument(
        "--client_config_file",
        type=Path,
        required=True,
